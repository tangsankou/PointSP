...
###modify: add sample_typle
def sample_and_group(npoint, radius, nsample, xyz, points,sample_type):
    """
    Input:
        npoint:
        radius:
    ...
    if sample_type == 'ffps':
        ...
        fps_idx,idx_k = weighted_random_point_sample(xyz, npoint, k=20)

    else:###fps

        fps_idx = pointnet2_utils.furthest_point_sample(xyz, npoint).long() # [B, npoint]

    new_xyz = index_points(xyz, fps_idx) ##xyz:(B,N,3)
    new_points = index_points(points, fps_idx)

    idx = knn_point(nsample, xyz, new_xyz) ###选周围点
