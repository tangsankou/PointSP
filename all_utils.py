...
class PerfTrackValomni:
    """
    Records epoch wise performance for validation
    """
    def __init__(self, task,selected_classes, extra_param=None):
        self.task = task
    ...
    def update(self, data_batch, out):
        if self.task in ['cls', 'cls_trans']:
            correct = self.get_correct_list(out['logit'], data_batch['label'],self.selected_classes)
            print("correct:",correct)
            self.all.extend(correct)
            self.update_class_see_corr(out['logit'], data_batch['label'],self.selected_classes)
        else:
            assert False
    def agg(self):  
        if self.task in ['cls', 'cls_trans']:
            print("class_corr:",self.class_corr)
            print("class_seen:",self.class_seen)
            # 过滤掉 class_seen 中为 0 的项，并同时过滤 class_corr 中对应的项  
            valid_indices = np.array(self.class_seen) != 0  
            class_corr_filtered = np.array(self.class_corr)[valid_indices]  
            class_seen_filtered = np.array(self.class_seen)[valid_indices]  
            print("class_corr_:",class_corr_filtered)
            print("class_seen_:",class_seen_filtered)
            # 计算类别准确率，只考虑在数据集中出现的类别  
            if class_seen_filtered.size > 0:  # 确保过滤后还有有效的类别  
                class_acc = np.mean(class_corr_filtered / class_seen_filtered)  
            else:  
                class_acc = np.nan  # 如果没有任何类别出现，则类别准确率为 NaN 
            ###origin
            perf = {
                'acc': self.get_avg_list(self.all),
                'class_acc': class_acc
    ...
    def update_class_see_corr(self, logit, label, selected_classes):  
        if self.class_seen is None:  
            # 初始化所有类别的seen和corr数组  
            num_class = logit.shape[1]  
            self.class_seen = [0] * num_class  
        ...
        selected_classes = torch.tensor(selected_classes, device=logit.device, dtype=torch.long)  
        mask = torch.zeros_like(logit, dtype=torch.bool)  
        mask[:, selected_classes] = True  
        masked_logit = logit.clone()  
        masked_logit[~mask] = float('-inf')

        pred_label = masked_logit.argmax(dim=1)
    ...
    @staticmethod
    def get_correct_list(logit, label, selected_classes):
        print("logit:",logit[0])
        selected_classes = torch.tensor(selected_classes, device=logit.device, dtype=torch.long)  
        mask = torch.zeros_like(logit, dtype=torch.bool)  
        ...
        masked_logit = logit.clone()  
        masked_logit[~mask] = float('-inf')
        pred_class = masked_logit.argmax(dim=1)
        print("label:",label)
        print("pred:",pred_class)
        correct = (pred_class == label).to('cpu').tolist()
        
        return correct
    @staticmethod
    def get_avg_list(all_list):
        print("all_list:",all_list[0])
        for x in all_list:
            assert isinstance(x, bool)
        return sum(all_list) / len(all_list)
