...
###weighted fps
class WfurthestPointSampling(Function):
    @staticmethod
    def forward(ctx, xyz, weights, npoint):
        # type: (Any, torch.Tensor, torch.Tensor, int) -> torch.Tensor
        r"""
        Uses iterative furthest point sampling to select a set of npoint features that have the largest
        minimum distance
        ...
        torch.Tensor
            (B, npoint) tensor containing the set
        """
        out = _ext.wfurthest_point_sampling(xyz, weights, npoint)
...
wfurthest_point_sample = WfurthestPointSampling.apply
