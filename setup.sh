...
:<<!
cd ${root}/pointnet2_pyt && ${PYTHON_BIN} -m pip install -e . && cd -
cd ${root}/PCT_Pytorch/pointnet2_ops_lib && ${PYTHON_BIN} -m build && ${PYTHON_BIN} -m pip install . && cd -
cd ${root}/emd && ${PYTHON_BIN} -m build && ${PYTHON_BIN} -m pip install . && cd -
cd ${root}/PyGeM && ${PYTHON_BIN} -m build && ${PYTHON_BIN} -m pip install . && cd -
!