...
# subset_list=[0,2,5,6,7,8,10,10,15,17,18,19,20,26,26,30,32,33,36,37]
def load_data(partition):
    h5_name = "/home/user_tp/workspace/data/OmniMN40up/OmniMN40.h5"
    f = h5py.File(h5_name)
    data = f['data']
...
def save_data(all_data, all_label, corruption_type, level):
    if not os.path.exists(os.path.join(DATA_DIR, 'omni_c')):
        os.makedirs(os.path.join(DATA_DIR, 'omni_c'))
    if corruption_type == 'clean':
        h5_name = os.path.join(DATA_DIR, 'omni_c', '{}.h5'.format(corruption_type))
    else:
        h5_name = os.path.join(DATA_DIR, 'omni_c', '{}_{}.h5'.format(corruption_type, level))
