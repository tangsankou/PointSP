...
def sample_and_group(npoint, radius, nsample, xyz, points,sample_type='fps', returnfps=False):
    """
    Input:
        npoint:
        radius:
    ...
    B,N,C= xyz.shape
    #add for sample_type
    # print("sag xyz:",xyz.shape)
    # print("sample and group:",sample_type)
    if sample_type == 'fps':
        idx_ = pointnet2_utils.furthest_point_sample(xyz, npoint).long()
    if sample_type == 'ffps':
        num1 = int(N * 0.95)
        ...
        weights[mask] = -1
        idx_ = pointnet2_utils.wfurthest_point_sample(xyz, weights, npoint).long()
    elif sample_type == 'wrs':
        idx_,_ = weighted_random_point_sample(xyz, npoint, k=20,replace=True)
    else:
        assert False
    new_xyz=index_points(xyz,idx_)
    ###the origin new_xyz
...
class CIC(nn.Module):
    ...
    def __init__(self, npoint, radius, k, in_channels, output_channels, bottleneck_ratio=2, mlp_num=2, curve_config=None,sample_type='fps'):
        ...
        self.maxpool = MaskedMaxPool(npoint, radius, k,sample_type=self.sample_type)
...
class MaskedMaxPool(nn.Module):
    def __init__(self, npoint, radius, k,sample_type='fps'):
    ...
    def forward(self, xyz, features):
        sub_xyz, neighborhood_features = sample_and_group(self.npoint, self.radius, self.k, xyz, features.transpose(1,2),self.sample_type)
