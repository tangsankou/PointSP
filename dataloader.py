...
###add for pointcloudc
def load_data_h5(file_path, corruption, severity):
    # 读取 HDF5 文件
    
    DATA_DIR = os.path.join(file_path, corruption + '_' +str(severity) + '.h5')

    with h5py.File(DATA_DIR, 'r') as h5_file:
        # 读取数据集
        data = np.array(h5_file['data']).astype(np.float32)#和npy文件的数据格式一致
        labels = np.array(h5_file['label']).astype(np.uint8)
    return data, labels
...
class PointCloudC(Dataset):
    def __init__(self, split, test_data_path,corruption,severity):
        ...
        self.data, self.label = load_data_h5(self.data_path, self.corruption, self.severity)
        # self.num_points = num_points
        self.partition =  'test'
...
class OmniMN40(Dataset):
    def __init__(self, split, test_data_path,corruption,severity):
        ...
        with h5py.File(test_data_path, 'r') as h5_file:
            # 读取数据集
            self.data = np.array(h5_file['data']).astype(np.float32)#和npy文件的数据格式一致
            self.label = np.array(h5_file['label']).astype(np.uint8)
        # self.num_points = num_points
        point_clouds=[pc_normalize(pcd) for pcd in self.data]
        self.data= np.array(point_clouds).astype('float32')
...
def create_dataloader(split, cfg):
    ...
    ###modify add for pointcloud_c and OmniMN40
    elif cfg.EXP.DATASET == "pointcloud_c":
        dataset_args.update(dict(**cfg.DATALOADER.MODELNET40_C))
        dataset = PointCloudC(**dataset_args)
    elif cfg.EXP.DATASET == "OmniMN40":
        dataset_args.update(dict(**cfg.DATALOADER.MODELNET40_C))
        dataset = OmniMN40(**dataset_args)
    else:
        assert False
...
if __name__ == '__main__':
    data_path = "/home/user_tp/workspace/code/attack/ModelNet40-C/data/modelnet40_c"
    corruption = "density"
    severity = 5
    pc,_=load_data(data_path,corruption,severity)
    print("pc:",pc.shape)