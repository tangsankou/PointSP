...
def mixup(data_batch,cfg):
    ...
    mix_rate_expand_xyz = mix_rate.expand(data_batch['pc'].shape).to(device)
    ...
    data_batch['lam'] = mix_rate.clone().squeeze_()
    return data_batch
...
def rsmix(data, cfg, n_sample=512, KNN=False):
    ...
    data_batch = data['pc'].cpu().numpy()
    label_batch = data['label'].cpu().numpy()
